#!/usr/bin/env python3
"""Plot benchmark results: Total TPS vs batch size for selected methods."""
...
# ── Data ──
# Job-level TPS (Total Throughput)
job_tps = {
    "AIME 2022-2024": {
...
def plot_dataset(data, dataset_name, ylabel, filename_prefix):
    """Plot one dataset."""
    fig, ax = plt.subplots(figsize=(10, 6))
    x = np.arange(len(bs_list))

    for method in methods:
        if method in data[dataset_name]:
            vals = data[dataset_name][method]
            ax.plot(x, vals,
    ...
    ax.set_xlabel("Batch Size (Concurrency)", fontsize=13)
    ax.set_ylabel(ylabel, fontsize=13)
    ax.set_title(f"{dataset_name} — {ylabel}", fontsize=14, fontweight='bold')
    ax.legend(fontsize=10, loc='upper left')
    ax.grid(True, alpha=0.3)
    ax.set_yscale('log', base=2)

    # Custom y ticks
    ymin = min(min(v) for v in data[dataset_name].values())
    ymax = max(max(v) for v in data[dataset_name].values())
    yticks = [128, 256, 512, 1024, 2048, 4096, 8192]
    yticks = [y for y in yticks if ymin * 0.8 <= y <= ymax * 1.2]
    ax.set_yticks(yticks)
    ax.set_yticklabels([str(y) for y in yticks])

    plt.tight_layout()
    fname = f"{OUT_DIR}/{filename_prefix}_{dataset_name.lower().replace(' ', '_').replace('-', '_')}.png"
    plt.savefig(fname, dpi=150)
    plt.close()
    print(f"Saved: {fname}")

def plot_all_datasets_combined(data, ylabel, filename):
    """2x2 grid of all datasets."""
    ...
    for idx, (ax, ds) in enumerate(zip(axes.flat, datasets)):
        ...
        ymin = min(min(v) for v in data[ds].values())
        ymax = max(max(v) for v in data[ds].values())
        yticks = [128, 256, 512, 1024, 2048, 4096, 8192]
        yticks = [y for y in yticks if ymin * 0.8 <= y <= ymax * 1.2]
        ax.set_yticks(yticks)
        ax.set_yticklabels([str(y) for y in yticks])
...
def plot_speedup_over_ar(data, filename):
    """2x2 grid: speedup over AR."""
    fig, axes = plt.subplots(2, 2, figsize=(18, 12))
    datasets = ["AIME 2022-2024", "MATH-500", "ShareGPT", "Synthetic"]

    for idx, (ax, ds) in enumerate(zip(axes.flat, datasets)):
        x = np.arange(len(bs_list))
        ar_vals = np.array(data[ds]["AR"])

        for method in methods:
            if method == "AR":
                continue
            if method in data[ds]:
                vals = np.array(data[ds][method])
        ...
        ax.set_xticklabels([str(b) for b in bs_list])
        ax.set_xlabel("Batch Size", fontsize=11)
        ax.set_ylabel("Speedup over AR", fontsize=11)
        ax.set_title(ds, fontsize=13, fontweight='bold')
        ax.grid(True, alpha=0.3)
        ax.set_ylim(0.5, 3.0)
...
for ds in ["AIME 2022-2024", "MATH-500", "ShareGPT", "Synthetic"]:
    plot_dataset(job_tps, ds, "Total Throughput (tok/s)", "job_tps")
    plot_dataset(per_req_tps, ds, "Per-Request TPS (tok/s)", "per_req_tps")

# Combined 2x2 plots
plot_all_datasets_combined(job_tps, "Total Throughput (tok/s)", "job_tps_all.png")
plot_all_datasets_combined(per_req_tps, "Per-Request TPS (tok/s)", "per_req_tps_all.png")

# Speedup over AR
plot_speedup_over_ar(job_tps, "speedup_over_ar.png")
